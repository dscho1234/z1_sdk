...
from envs.z1_env_rtc_wait import EEPoseCtrlCartesianCmdWrapper


def inference_function(observation, square_vertices, orientation_vertices, total_steps, current_step, sequence_length=16, inference_time=0.15):
    """
...
def main():
    ...
    try:
        ...
        current_action_sequence = inference_function(
            obs, square_vertices, orientation_vertices, total_steps, 0, sequence_length, inference_time
        ) # o_0 -> a_0, a_1, a_2, ...
        current_action_index = 0
        ...
        for step in range(total_steps):
            ...
            # Note: return values from wait=False are not meaningful
            env.step(action, wait=False) # a_t
            print(f"Step {step}: Started non-blocking execution")
            ...
            if step > 0 and step < total_steps - 1:  # Don't run inference for step 0 or last step
                ...
                latest_inference_result = inference_function(
                    obs, square_vertices, orientation_vertices, total_steps, step, sequence_length, inference_time
